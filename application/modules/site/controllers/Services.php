...
redirect(DIR."site/".$controller_curt."/".$curt_sub);    
    ...
    function index() {
        redirect(DIR."site/services/our_services");   
	}
	function our_services() {
        ...
        $tables = "our_services";
        $config['base_url'] = base_url().'site/services/our_services'; 
       ...
       // $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
		...
		$datahead['lang'] =$lang; 
		$data['lang'] =$lang;
		$data['result_amount'] =$this->db->order_by('id','desc')->get_where('our_services',array('view'=>'1'))->result(); 
        else:
	...
	function details_services() {
        ...
        if($tab_id==""){redirect(DIR."site/services/our_services");   }
            else {
		global $lang;
		if( isset($this->session->get_userdata('lang')['lang']) ){
			$lang = $this->session->get_userdata('lang')['lang'];
        ...
        $data["results"] =$this->db->order_by('id','desc')->get_where('our_services',array('view'=>'1'))->result(); 
		$datahead['site_info'] =$this->db->get_where('site_info')->result(); 
		$data['siteinfo']=$this->db->get_where('site_info')->result();
		$datahead['lang'] =$lang; 
		$data['lang'] =$lang;
		...
		$this->load->view('include/head',$datahead);
		$this->load->view('include/insideheader',$datahead);
		$this->load->view('details_services',$data);
        $this->load->view('include/footer',$datahead);
            }
    }
