			...
			<ul class="list-unstyled tab-switch ">
				...
				<?php if($result_amount>0){
						...
						foreach($results as $prod) {
							$count++;
						?>
						<li <?php if($count==1){ ?> class="selected" <?php }?> data-class=".tab-<?=$prod->id?>">
							 <?php
							 echo ( $lang == 'arabic' )?$prod->title_ar: $prod->title_eng ; 
								?> 
	<i class="fas  <?php echo ( $lang == 'arabic' )? "fa-angle-left": "fa-angle-right";?>  fa-lg pull-left"></i></li>
						<?php }  }?>
			</ul>
		</div>
		<div class="col-md-8">
			...
			<?php if($result_amount>0){
				$count_r=0;
						...
						foreach($result_amount as $service_details) {
							$count_r++;
						?>
						<div class="tab-<?=$service_details->id?> <?php if($count_r==1){?>tab-one<?php }?>">
						<?php
