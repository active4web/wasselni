...
class Api extends API_Controller
{
	...
	public function send_sms()
    {
        ...
        if($this->form_validation->run() === FALSE){
          ...
          $verification_info = get_this('customers',['phone'=>$this->input->post('phone'),'status'=>'0']);
			...
			$islam = 'success';
    		if($islam == 'success'){
				$data['message'] = lang('Success code sent successfully');
				$data['errNum'] = 405;
                ...
                if ($customer_info['status'] == 0) {
					...
					'token' => $token
                     ...
                     if ($customer_info['status'] == 1) {
						...
						$data['message'] = lang('Account activated');
						$data['errNum'] = 405;
            ...
            $customer = get_this('customers',['id'=>$this->input->post('customer_id')]);
                 if ($customer) {
                     if ($customer['status'] == '1') {
                              ...
                              $data['customer_info'] = $customer_info;
                              $data['message'] = lang('Operation completed successfully');
                              $data['errNum'] = 405;
                              $this->api_return([
								'status' => true,
								"result" => $data
        ...
        if($this->form_validation->run() === FALSE){
          ...
          if ($user_info) {
              if ($user_info['status'] == '1') {
                      ...
                      if ($ticket) {
							if($lang=='arabic' || $lang=="" || $lang!="english"){
								$title = get_this('tickets_types',['id' => $ticket['ticket_type_id']],'name_ar');
							}else{
								$title = get_this('tickets_types',['id' => $ticket['ticket_type_id']],'name_en');
							}
                            $result = [
                                            'ticket_id' => $ticket['id'],
                                            'title'     => $title,
			...
			if ($user_info) {
				...
				$share = get_this('customers',['id' =>$this->input->post('customer_id'),'invitation_code'=>$this->input->post('code')],'invitation_count');
					if(isset($share)){
						...
						$this->Main_model->update('customers',['id'=>$this->input->post('customer_id')],$update);
						$data['message'] = lang('A post has been made and a point deducted from your calling points');
                        $data['errNum'] = 405;
							  $this->api_return([
									'status' => true,
									"result" => $data
	...
	public function mazad_now()
    {
		...
		foreach ($mazadat as $item) {
			...
			////////////////////////////////////////////////////////////////
			$result['message']['message'] = lang('Operation completed successfully');
			$result['message']['errNum'] = 405;
			$result['message']['total'] = count($total);
			$result[] = [
				'mazad_id'         => $item->id,
