...
class Api extends API_Controller
{
		...
		if($this->form_validation->run() === FALSE){
			...
			if ($mazad_info) {
				...
				if($lang=='arabic' || $lang=="" || $lang!="english"){
					...
					$data['mazad']['product_detailes']['desc'] = $data['mazad']['product_detailes']['desc_ar'];
					...
					$data['mazad']['product_detailes']['desc'] = $data['mazad']['product_detailes']['desc_en'];
					unset($data['mazad']['product_detailes']['name_en']);
					unset($data['mazad']['product_detailes']['desc_en']);
					///////////////////////////////////////////////////////////////
				}
				$data['mazad']['product_detailes']['path'] = base_url('uploads/customers/');
				...
				if ($mazad_info) {
					...
					if($customer_points < $mazad_info['pluse_points']){
						...
						if($now >= $compare and $now <= $selectedTime){
							...
							$e_time0 = $split[0];
							$e_time1 = $split[1];
							$e_time = strtotime("+10 seconds", strtotime($e_time1));
	...
	public function confirm_points()
    {
		...
		$this->form_validation->set_rules('customer_id', lang('Customer ID'), 'trim|required|numeric');
		$this->form_validation->set_rules('confirm', lang('Confirm'), 'trim|required|numeric');
		$this->form_validation->set_rules('points', lang('Points'), 'trim|required|numeric');
		...
		if($this->form_validation->run() === FALSE){
			...
			if(form_error('confirm')){
				if($this->input->post('confirm')==="" || !$this->input->post('confirm')){
					$data[] = array('message'=> strip_tags(form_error('confirm')),"errNum" => 0);
				}else{
					$data[] = array('message'=> strip_tags(form_error('confirm')),"errNum" => 1);
				}
			}
			
			if(form_error('points')){
				if($this->input->post('points')==="" || !$this->input->post('points')){
					$data[] = array('message'=> strip_tags(form_error('points')),"errNum" => 0);
				}else{
					$data[] = array('message'=> strip_tags(form_error('points')),"errNum" => 1);
				}
			}
			//print_r($data);die;
			$this->api_return([
			...
			if($this->input->post('confirm')!=0){
				...
				$store['points'] = $old_points + $this->input->post('points');
                $update = $this->Main_model->update('customers',['id'=>$user_info['id']],$store);
				...
				$this->api_return([
				'message' => lang('Sorry payment process has not been successfully please try again'),
				'errNum' => 406,
				'status' => false,
				],200);
			}
