	...
	public function register()
    {
        ...
        if($this->form_validation->run() === FALSE){
            ...
            $this->api_return([
				...
				$msgKey = "(1),*,$reciever,@,(2),*,$activation_code";
				$numbers = $reciever;
				...
				/////////////////////////////////////////////////////////
					...
					/*if($lang=='arabic' || $lang=="" || $lang!="english"){
						$message = array('MessageAr'=>lang('Success code sent successfully'),'MessageEn'=>"");
					}else{
						$message = array('MessageAr'=>"",'MessageEn'=>lang('Success code sent successfully'));
					}
					$this->api_return([
						'message' => $message,
						'errNum' => 405,
	            ...
	            if($insert){
					...
					$result = new stdClass();
					if($lang=='arabic' || $lang=="" || $lang!="english"){
						$message = $message_info->Error->MessageAr;
					}else{
						$message = $message_info->Error->MessageEn;
					}
					$this->api_return([
						'message' => $message,
						"errNum" => 3,
		...
		if($phone){
          $verification_info = get_this('customers',['phone'=>$this->input->post('phone'),'status'=>'0']);
			...
			if($message_info->ResponseStatus == 'success'){
				...
				$result = new stdClass();
				//"message" => $message_info,
				//'message' => array('MessageAr'=>$message_info->Error->MessageAr,'MessageEn'=>$message_info->Error->MessageEn),
				$this->api_return([
					'message' => $message,
					"errNum" => 3,
					'status' => false,
