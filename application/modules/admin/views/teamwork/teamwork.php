			...
			<div class="page-content-wrapper">
				...
				<div class="page-content">
					...
					<ul class="page-breadcrumb breadcrumb">
						...
						<li>
							<span class="active">الصيدليات</span>
						</li>
					</ul>
					<!-- END PAGE BREADCRUMB -->
...
$pharmaceutical=$this->db->get_where("pharmaceutical",array('id_pharmacy'=>$data->id))->result();

$total_order=$this->db->get_where("order_requested",array('pharmacy_id'=>$data->id))->result();
...
?>
	...
	<tr class="odd gradeX">
							...
							<td><?=mb_substr($data->name,0,15);?></td>
							
									<td> <?=$data->username;?> </td>
										<td> <?=  count($total_order);?> </td>
									<td> <?=$data->depart;?> </td>
							<td> <?=$data->phone;?> </td>
							<td><a href="<?= base_url()?>admin/teamwork/pharmaceutical?id=<?= $data->id?>" target="_blank"> الأدوية (<?= count($pharmaceutical)?>)</a></td>
						<td> <?= $main_date;?></td>
												<td>
												    
<a href="<?= DIR?>admin/teamwork/all_pharmacy_user?id=<?=$data->id_admin?>" target="_blank" title="الخريطة"> <?= $agent_user?></a>

												    
												</td>
													<td>
												    
<a href="<?= DIR?>admin/clients/add_ownerpharmacy?id=<?=$data->id?>" target="_blank"> المدير</a>
