...
<title>طلبات التصوير والتصميم</title>
...
<li>
						...
						<li>
							<span class="active">طلبات التصميم والتصوير</span>
						</li>
					</ul>
					<!-- END PAGE BREADCRUMB -->
						...
						<div class="col-md-12">
							...
							<div class="portlet">
								...
								<span class="portlet-body">
									...
									<form action="<?= base_url()?>admin/products/delete_photography" method="POST" id="form">
									<div>
									...
									<table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1_2">
                                        ...
                                        <?php
                                            foreach($results as $data) {
												...
												switch($view){
													case 0:
													  $view="<span class='label label-sm label-danger'>غير مقرواءه</span>";
													  break;
													case 1:
													  $view="<span class='label label-sm label-success'>مقرواءه</span>";
													  break;
													default:
													  break; 
												}
												$id_service=$data->id_service;
												$name_cat=get_table_filed("team_work",array("id"=>$id_service),"name");
                                        ...
                                        ?>
											<tr class="odd gradeX">
												...
												<td> <?= mb_substr($data->name,0,30);;?> </td>
													<td> <?= mb_substr($name_cat,0,30);?> </td>
												<td> <?= mb_substr($data->phone,0,30);;?> </td>
												<td> <?= mb_substr($data->whatsapp,0,30);;?> </td>
												<td> <?=$data->date	;?> </td>
												<td>
												...
												<td>
													<div class="btn-group">
														...
														<ul class="dropdown-menu pull-left" role="menu">
															<li><a href="<?= base_url()?>admin/products/details_photography?id=<?=$data->id;?>"><i class="fa fa-eye"></i>التفاصيل</a></li>
															<li><a href="<?= base_url()?>admin/products/delete_photography?id_notifications=<?=$data->id;?>"><i class="fa fa-remove"></i> حذف </a></li>
															<li><a href="<?= base_url()?>admin/products/notification_photography?id_notifications=<?=$data->id;?>"><i class="fa fa-bell"></i> ارسال تنبيه </a></li>
...
$(document).ready(function(e) {
    ...
    $(".addbutton").click(function(e) {
        window.location.assign("<?= DIR?>admin/products/add_departments");
    });
...
var data={id:id};
			$.ajax({
				url: '<?php echo base_url("admin/products/check_view_photography") ?>',
                type: 'POST',
                data: data,				
                success: function( data ) {
                	if (data == "1") {
					// alert(data);
                		$(".code_actvation-"+id).html("<span class='label label-sm label-success'>مقرواءه</span>");
                	}
                	if (data == "0") {
                		$(".code_actvation-"+id).html("<span class='label label-sm label-danger'>غير مقرواءه</span>");
                	}
				}
         });
	});
